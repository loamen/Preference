...
class SliderPreference @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = R.attr.sliderPreferenceStyle, defStyleRes: Int = 0) :
    ...
    init {
        ...
        val sliderAttributes = context.obtainStyledAttributes(attrs, MR.styleable.Slider, defStyleAttr, defStyleRes)
        sliderAttributes.let {
            valueFrom = it.getFloat(MR.styleable.Slider_android_valueFrom, 0F)
            valueTo = it.getFloat(MR.styleable.Slider_android_valueTo, 100F)
            stepSize = it.getFloat(MR.styleable.Slider_android_stepSize, 0F)
            labelBehavior = it.getInt(MR.styleable.Slider_labelBehavior, 0)
            value = it.getFloat(MR.styleable.Slider_android_value, 0F)
            it.recycle()
        }

        // Get SeekBarPreference attributes from XML
        val seekbarAttributes = context.obtainStyledAttributes(attrs, PR.styleable.SeekBarPreference, defStyleAttr, defStyleRes)
        seekbarAttributes.let {
            //seekBarIncrement = a.getInt(R.styleable.SeekBarPreference_seekBarIncrement, 0)
            isAdjustable = it.getBoolean(PR.styleable.SeekBarPreference_adjustable, true)
            mShowSeekBarValue = it.getBoolean(PR.styleable.SeekBarPreference_showSeekBarValue, false)
            updatesContinuously = it.getBoolean(PR.styleable.SeekBarPreference_updatesContinuously, false)
            it.recycle()
        }
